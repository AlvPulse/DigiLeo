...
import numpy as np
import copy
from src.dl_models import PyTorchClassifier, AudioCNN, AudioDNN, SaraCNN, AudioLSTM, AudioCRNN
...
class ModelFactory:
    ...
    def get_model(model_type, params):
        ...
        Args:
            model_type (str): 'rf', 'svm', 'log_reg', 'cnn', 'dnn', 'ensemble', 'sara_cnn', 'lstm', 'crnn'
        ...
        if model_type == 'rf':
            ...
            wrapper_params = {k: v for k, v in params.items() if k in ['epochs', 'batch_size', 'lr', 'device']}
            model_arch_params = {k: v for k, v in params.items() if k not in wrapper_params}
            return PyTorchClassifier(AudioCRNN, model_arch_params, **wrapper_params)
